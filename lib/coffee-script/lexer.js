...
(function() {
  ...
  exports.Lexer = Lexer = (function() {
    ...
    Lexer.prototype.identifierToken = function() {
      ...
      if (tag === 'IDENTIFIER' && (includes(JS_KEYWORDS, id) === true || includes(COFFEE_KEYWORDS, id) === true)) {
        tag = id.toUpperCase();
        if (tag === 'WHEN' && (ref4 = this.tag(), includes(LINE_BREAK, ref4) === true)) {
          tag = 'LEADING_WHEN';
        } else if (tag === 'FOR') {
          this.seenFor = true;
        } else if (tag === 'UNLESS') {
          tag = 'IF';
        } else if (includes(UNARY, tag) === true) {
          tag = 'UNARY';
        } else if (includes(RELATION, tag) === true) {
      ...
      if (tag === 'IDENTIFIER' && includes(RESERVED, id) === true) {
        this.error("reserved word '" + id + "'", {
          length: id.length
        });
      }
      if (tag !== 'PROPERTY') {
        if (includes(COFFEE_ALIASES, id) === true) {
          alias = id;
    ...
    Lexer.prototype.regexToken = function() {
      ...
      switch (false) {
        ...
        case !(match = REGEX.exec(this.chunk)):
          ...
          ref2 = this.tokens, prev = ref2[ref2.length - 1];
          if (prev) {
            if (prev.spaced && (ref3 = prev[0], includes(CALLABLE, ref3) === true)) {
              if (!closed || POSSIBLY_DIVISION.test(regex)) {
                return 0;
              }
            } else if (ref4 = prev[0], includes(NOT_REGEX, ref4) === true) {
              return 0;
            }
          }
          if (!closed) {
    ...
    Lexer.prototype.outdentToken = function(moveOut, noNewlines, outdentLength) {
      ...
      while (moveOut > 0) {
        ...
        if (!lastIndent) {
          ...
          if (outdentLength && (ref2 = this.chunk[outdentLength], includes(INDENTABLE_CLOSERS, ref2) === true)) {
    ...
    Lexer.prototype.literalToken = function() {
      ...
      tag = value;
      ref2 = this.tokens, prev = ref2[ref2.length - 1];
      if (prev && includes(['='].concat(slice.call(COMPOUND_ASSIGN)), value) === true) {
        skipToken = false;
      ...
      if (value === ';') {
        this.seenFor = false;
        tag = 'TERMINATOR';
      } else if (includes(MATH, value) === true) {
        tag = 'MATH';
      } else if (includes(COMPARE, value) === true) {
        tag = 'COMPARE';
      } else if (includes(COMPOUND_ASSIGN, value) === true) {
        tag = 'COMPOUND_ASSIGN';
      } else if (includes(UNARY, value) === true) {
        tag = 'UNARY';
      } else if (includes(UNARY_MATH, value) === true) {
        tag = 'UNARY_MATH';
      } else if (includes(SHIFT, value) === true) {
        tag = 'SHIFT';
      } else if (includes(LOGIC, value) === true || value === '?' && (prev != null ? prev.spaced : void 0)) {
        tag = 'LOGIC';
      } else if (prev && !prev.spaced) {
        if (value === '(' && (ref5 = prev[0], includes(CALLABLE, ref5) === true)) {
          if (prev[0] === '?') {
            prev[0] = 'FUNC_EXIST';
          }
          tag = 'CALL_START';
        } else if (value === '[' && (ref6 = prev[0], includes(INDEXABLE, ref6) === true)) {
          tag = 'INDEX_START';
          switch (prev[0]) {
            case '?':
              prev[0] = 'INDEX_SOAK';
  ...
  isUnassignable = function(name, displayName) {
    ...
    switch (false) {
      case includes(slice.call(JS_KEYWORDS).concat(slice.call(COFFEE_KEYWORDS)), name) === false:
        return "keyword '" + displayName + "' can't be assigned";
      case includes(STRICT_PROSCRIBED, name) === false:
        return "'" + displayName + "' can't be assigned";
      case includes(RESERVED, name) === false:
  ...
  JS_KEYWORDS = ['true', 'false', 'null', 'this', 'new', 'delete', 'typeof', 'in', 'instanceof', 'return', 'throw', 'break', 'continue', 'debugger', 'yield', 'await', 'if', 'else', 'switch', 'for', 'while', 'do', 'try', 'catch', 'finally', 'class', 'extends', 'super'];
