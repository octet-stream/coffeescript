...
(function() {
  ...
  UTILITIES = {
    ...
    asyncWrap: function() {
      return "(function(){ function typeOf(value) { return {}.toString.call(value).slice(8, -1).toLowerCase(); } function isPromise(obj) { return typeOf(obj) === 'promise'; } function isGeneratorFunction(fn) { return typeOf(fn) === 'generatorfunction'; } return function(gen) { return function() { var args = [].slice.call(arguments), ctx = this; return new Promise(function(_res, _rej) { if (isGeneratorFunction(gen)) gen = gen.apply(ctx, args); resolve(); function resolve(res) { try { var nextYield = gen.next(res); } catch (err) { return _rej(err); } __next(nextYield); } function reject(err) { try { var nextYield = gen.throw(err); } catch (e) { return _rej(e); } __next(nextYield); } function __next(nextYield) { if (nextYield.done) return _res(nextYield.value); var value = nextYield.value; if (value && isPromise(value)) value.then(resolve, reject); } }); }; }; })()";
    }
  };
