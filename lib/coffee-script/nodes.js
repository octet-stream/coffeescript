...
(function() {
  ...
  UTILITIES = {
    ...
    asyncWrap: function() {
      return "(function(){ function typeOf(value) { return {}.toString.call(value).slice(8, -1).toLowerCase(); } function isPromise(obj) { return typeOf(obj) === 'promise'; } return function(fn) { return function asyncWrap() { var gen = fn.apply(this, arguments); return new Promise(function(resolve, reject) { function onFulfilled(res) { try { var nextYield = gen.next(res); } catch (err) { return reject(err); } next(nextYield); } function onRejeted(err) { try { var nextYield = gen.throw(err); } catch (err) { return reject(err); } next(nextYield); } function next(nextYield) { if (nextYield.done) { return resolve(nextYield.value); } var value = nextYield.value; if (value && isPromise(value)) { return Promise.resolve(value).then(onFulfilled, onRejeted); } return onRejeted(new TypeError('You may use only Promise.')); } return onFulfilled(); }); }; }; })()";
    }
  };
